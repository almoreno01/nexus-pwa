...
async function loadSummary(doc){
  ...
  state.summary[doc.id]={
    totals,workers,sheetNames,daysInMonth,
    month:monthDate?monthDate.getMonth()+1:null,
    year:monthDate?monthDate.getFullYear():null,
    error:realSheetCount===0
  };
...
async function loadAllSummaries(){
  for(const doc of docs)await loadSummary(doc);
}