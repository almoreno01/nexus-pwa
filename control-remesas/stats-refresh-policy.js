...
async function spreadsheetIsPublic(url){
  const id=extractId(url);if(!id)return false;
  try{
    const endpoint="https://docs.google.com/spreadsheets/d/"+encodeURIComponent(id)+"/gviz/tq?tqx=out:json&headers=0&range=A1:A1";
    const r=await fetch(endpoint,{cache:"no-store"});if(!r.ok)return false;
    const text=await r.text();
    if(/<!doctype html|<html/i.test(text)||/sign in|iniciar sesi[oó]n|request access|solicitar acceso/i.test(text))return false;
    const start=text.indexOf("{"),end=text.lastIndexOf("}");if(start<0||end<=start)return false;
    const payload=JSON.parse(text.slice(start,end+1));return !payload.status||payload.status==="ok";
  }catch(e){return false;}
}
function showPublicAccessWarning(){
  document.getElementById("publicAccessWarning")?.remove();
  const bg=document.createElement("div");bg.className="modal-bg";bg.id="publicAccessWarning";
...
document.addEventListener("click",e=>{if(e.target.id==="closePublicAccessWarning"||e.target.id==="publicAccessWarning")document.getElementById("publicAccessWarning")?.remove();},true);
async function refreshDocument(doc){if(!doc||state.loading[doc.id])return;await loadSummary(doc);}
...
document.addEventListener("click",async function(e){
  ...
  if(t.id==="refreshAllBtn"||t.closest?.("#refreshAllBtn")){
    e.preventDefault();e.stopImmediatePropagation();await refreshAllDocuments();return;
  }
  if(t.id==="saveBtn"){
    e.preventDefault();e.stopImmediatePropagation();
    const name=capName(document.getElementById("docName")?.value||"");
    const url=(document.getElementById("docUrl")?.value||"").trim();
    if(!name||!extractId(url)){alert("Escribe un nombre y un enlace válido de Google Sheets.");return;}
  ...
  if(t.id==="saveEditBtn"){
    ...
    if(!name||!extractId(url)){alert("Escribe un nombre y un enlace válido de Google Sheets.");return;}
    const d=docs.find(x=>x.id===state.current?.id);if(d){d.name=name;d.url=url;save();}goHomeReplace();
  }
},true);